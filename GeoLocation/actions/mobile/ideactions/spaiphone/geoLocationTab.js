...
function geoSuccessCallBackTab(position) {
    ...
    var speed;
    if (lat == null) frmOptions.lblLatValue.text = "= unavailable.";
    else frmOptions.lblLatValue.text = "= " + lat;
    if (lang == null) frmOptions.lblLongValue.text = "= unavailable.";
    else frmOptions.lblLongValue.text = "= " + lang;
    if (alt == null) frmOptions.lblAltValue.text = "= unavailable.";
    else frmOptions.lblAltValue.text = "= " + alt;
    if (acry == null) frmOptions.lblAccValue.text = "= unavailable.";
    else frmOptions.lblAccValue.text = "= " + acry;
    if (hedin == null) frmOptions.lblHeadValue.text = "= unavailable.";
    else frmOptions.lblHeadValue.text = "= " + hedin;
    if (kony.os.deviceInfo().name == "iPad") {
        speed = position.coords.speed;
        if (speed == null) frmOptions.lblSpeedValue.text = "= unavailable.";
        else frmOptions.lblSpeedValue.text = "= " + speed;
    } else {
...
function geoPositionTab() {
    ...
    if (kony.os.deviceInfo().name == "thinclient" || kony.os.deviceInfo().name == "Windows 8") hboxHeadspa.lblHeadspa.text = "Current Position";
    watchFlag = false;
...
function watchPositionTab() {
    if (kony.os.deviceInfo().name == "thinclient" || kony.os.deviceInfo().name == "Windows 8") hboxHeadspa.lblHeadspa.text = "Watch Position";
    kony.application.showLoadingScreen("loadingscreen", "Loading...", constants.LOADING_SCREEN_POSITION_FULL_SCREEN, true, false, null);
    var positionoptions = new Object();
    positionoptions.enablehighaccuracy = true;
...
function clearWatchTab() {
    ...
    if (kony.os.deviceInfo().name == "thinclient" || kony.os.deviceInfo().name == "Windows 8") hboxHeadspa.lblHeadspa.text = "Clear Watch";
...
function onClickOfSegApiTab(eventobj) {
    ...
    if (apiKey == 0) geoPositionTab();
    else if (apiKey == 1) watchPositionTab();
}
/*****************************************************************
...
function onHideFrmGeoTab() {
    if (watchFlag == true) clearWatchTab();
}
/*****************************************************************
...
function segClickEvent(seguiWidget, sectionIndex, rowIndex) {
    ...
    if (frmOptions.segNavigation.selectedItems[0]["lblGetCurrentPosition"] == "getCurrentLocation") geoPositionTab();
    else watchPositionTab();
}